...
export default function FarmerList({
  ...
  const filteredFarmers = useMemo(() => {
    return farmers.filter((f) => {
      ...
      const nik = f.nik || "";
      // Pencarian tetap mencakup NIK asli
      const matchesSearch =
      ...
      const matchesGroup =
        !filterGroup || f.farmer_group_id === Number(filterGroup);
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <div className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm space-y-3.5 w-full box-border">
        ...
        <div className="grid grid-cols-3 gap-2 w-full">
          <button
            ...
            className={`w-full py-1 rounded-lg text-sm font-medium cursor-pointer transition flex items-center justify-center gap-2 whitespace-nowrap border h-10 ${
          ...
          <button
            type="button"
            onClick={onSync}
            className="bg-[#86c295] hover:bg-[#94c286] cursor-pointer text-white w-full py-1 rounded-lg text-sm font-medium transition flex items-center justify-center gap-2 whitespace-nowrap h-10"
          >
          ...
          <button
            type="button"
            onClick={onInitAdd}
            className="bg-[#107349] hover:bg-[#179661] cursor-pointer text-white w-full py-1 rounded-lg text-sm font-medium transition flex items-center justify-center gap-2 whitespace-nowrap h-10"
          >
      ...
      {/* DAFTAR DATA PETANI */}
      <div className="space-y-3 max-h-145 overflow-y-auto pr-1 w-full">
        {filteredFarmers.map((farmer) => {
          const name = farmer.user?.name || farmer.name;
          const groupName =
            farmer.farmer_group?.name || "Belum Ada Kelompok Tani";
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-zinc-400 font-medium">
                      NIK: {formatMaskedNik(farmer.nik)}
                    </p>
                    ...
                    <p className="text-[11px] text-zinc-400 font-medium mt-1">
                      ...
                      ({farmer.total_land_area || 0} Ha)
                    </p>
                  </div>
                </div>
                <span
                  className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                    farmer.status === "Data Belum Tersinkronisasi"
        ...
        {filteredFarmers.length === 0 && (
          <p className="text-center text-sm text-zinc-400 py-6 font-medium">
