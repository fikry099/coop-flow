...
class CooperativeRegistrationController extends Controller
{
    ...
    {
        // Menggunakan $request->validate() agar standar Laravel ValidationException terpanggil
        ...
        try {
            ...
            // Jika dalam lingkungan testing, lempar exception agar PHPUnit dapat menampilkan detail error
    ...
    /**
     * API KOPERASI AKTIF (Akses: Kemenko Pangan)
     */
    public function getActiveRegistrations()
    {
        $activeRegistrations = User::role('petugas-koperasi')
            ->where('status', 'ACTIVE')
            ->with('cooperative') 
            ->latest()
            ->get()
